...
- Exceptions to the variable an function naming rules are made for alerting functions and colors, e.g. `notice "Some log item: ${blue}blue text${reset}` where `notice` is a function and `$blue` and `$reset` are global variables but are lowercase.
